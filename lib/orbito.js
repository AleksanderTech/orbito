...
import { treeToList, copyPublicAssets, runCommand, convertUserPath } from "./orbito-common.js";
import { pagePlugin } from "./orbito-esbuild.js";
  ...
  async pages(path) {
    ...
    for (const filename of await readdir(path)) {
      if (new RegExp(`\.(mjs|js|ts)$`).test(filename)) {
        const imports = await import(pathToFileURL(p.join(process.cwd(), path, filename)));
